...
(function() {
  jQuery(function() {
    ...
    $('.addToCart').on('click', function(event) {
      ...
      return $.ajax({
        ...
        success: function(data, status, response) {
          ...
          return toastr.success('Item successfully added to cart', {
            timeOut: 1000
          });
        }
      });
    ...
    $('.removeFromCart').on('click', function(event) {
      var item_id, parent, parents_parent, updateTotal;
      console.log($('#totalPrice').text());
      item_id = $(this).data('id');
      parent = $(this).parent();
      parents_parent = parent.parent();
      updateTotal = function(origTotal, itemPrice) {
        var newTotal;
        newTotal = origTotal - itemPrice;
        return $('#totalPrice').text(newTotal);
      };
      return $.ajax({
        ...
        success: function(data, status, response) {
          ...
          $('#cartItems').text(data.cart_item_count);
          return toastr.success('Item successfully removed from cart', {
            timeOut: 1000
          });
        }
      });
    });
    cb_toggle = function(checkboxes) {
      return $('.selectAll').on('click', function(event) {
        if ($(this).prop('checked') === true) {
          return $.each(checkboxes, function(checkbox) {
            return checkboxes[checkbox].checked = true;
          });
        } else {
          return $('input:checkbox').removeAttr('checked');
        }
      });
    };
    cb_toggle(document.getElementsByName('item_ids[]'));
    cb_toggle(document.getElementsByName('cc_ids[]'));
    $('#addMultipleToCart').on('click', function(event) {
      var item_ids;
      item_ids = Array();
      $('[name="cc_ids[]"]:checked').each(function() {
        return item_ids.push($(this).val());
      });
      if (item_ids.length === 0) {
        return sweetAlert("Warning!", "You have not selected any items to buy", "warning");
      } else {
        return $.ajax({
          url: "/add_to_cart",
          type: 'POST',
          ...
          success: function(data, status, response) {
            console.log(data);
            $('#cartItems').text(data.cart_item_count);
            return toastr.success('Items successfully added to cart', {
              timeOut: 1000
            });
          }
        });
    ...
    $('#deleteAll').on('click', function(event) {
      ...
      if (item_ids.length === 0) {
        return sweetAlert("Oops!", "Please select Items to delete", "error");
      } else {
        return $.ajax({
          ...
          success: function(data, status, response) {
            ...
            $.each(data.ids, function(key, value) {
              return $("row_" + value).hide();
            });
            return toastr.success('Items successfully deleted', {
              timeOut: 1000
            });
          }
        });
    ...
    return $('.reportBtn').on('click', function(event) {
      var item_id;
      item_id = $(this).data('id');
      return $("input[id=report_item_id]").val(item_id);
    });
  });
